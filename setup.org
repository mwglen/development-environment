...
Now you can use ~sudo systemctl hibernate~ to hibernate and ~sudo systmectl hybrid-sleep~ to sleep. I recommend aliasing these to ~hibernate~ and ~hybrid-sleep~. I also recommend changing the behavior of the system keys to all hibernate the computer
...
3. You should now find that fstrim set up a weekly timer in ~systemctl list-timers~.
   
* Configuration Files
** ZSH Config
:PROPERTIES:
:header-args:bash: :tangle (concat (getenv $ZDOTDIR) "/.zshrc")
:END:
*** General
#+begin_src bash
unsetopt BEEP
...
* Script
:PROPERTIES:
:header-args:bash: :tangle ./setup.sh
:END:
** Script Headers
*** Use Bash
#+begin_src bash
...
#+begin_src bash
$INSTALL nvidia nvidia-prime optimus-manager
#+end_src
...
*** Bluetooth
#+begin_src bash
$INSTALL bluez bluez-utils bluetooth-autoconnect pulseaudio-bluetooth
...
**** Environment Config
:PROPERTIES:
:header-args:bash: :tangle (concat (getenv $HOME) "/.zshenv")
:END:
#+begin_src bash
source $HOME/.profile
...
:PROPERTIES:
:header-args:conf: :tangle (concat (getenv $HOME) "/.mbsyncrc")
:END:
#+begin_src conf
IMAPAccount gmail
Host imap.gmail.com
...
******* env.sh
:PROPERTIES:
:header-args:sh: :tangle (concat (getenv $XDG_CONFIG_HOME) "/polybar/scripts/env.sh") (identity #o755)
:END:
#+begin_src sh
export REDSHIFT=on
...
******* redshift.sh
:PROPERTIES:
:header-args:sh: :tangle (concat (getenv $XDG_CONFIG_HOME) "/polybar/scripts/redshift.sh") (identity #o755)
:END:
#+begin_src sh
#!/bin/sh
...
*** Configure GTK
:PROPERTIES:
:header-args:python: :tangle (concat (getenv $XDG_CONFIG_HOME) "/gtk-3.0/settings.ini")
:END:
#+begin_src conf
[Settings]
...
***** EXWM Outer Gaps
#+begin_src bash
cd ~/.config/emacs/lisp && wget https://raw.githubusercontent.com/lucasgruss/exwm-outer-gaps/main/exwm-outer-gaps.el
#+end_src
...
myStartupHook = do
    spawnOnce "picom &"
    spawnOnce "feh --bg-scale $BACKGROUNDS/unit-01.jpg"
    spawnOnce "xmodmap $XDG_CONFIG_HOME/xmodmap/config"
...
#+begin_src bash
$INSTALL batsignal
systemctl enable batsignal
systemctl start batsignal
#+end_src
...
**** Apply Theme
#+begin_src bash
ln $DIR/doom-moonless-theme.el ~/.config/emacs/doom-moonless-theme.el
#+end_src
...
***** Configuration
:PROPERTIES:
:header-args:conf: :tangle (concat (getenv $XDG_CONFIG_HOME) "/nimble/nimble.ini")
:END:
#+begin_src conf
